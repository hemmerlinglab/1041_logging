...
def get_voltages():
    ...
    return voltage_room, voltage_cryo
...
def get_pressure(repeat=5, TOL=0.1):

    vols_room = np.zeros(repeat)
    vols_cryo = np.zeros(repeat)
    for i in range(repeat):
        vols_room[i], vols_cryo[i] = get_voltages()

    voltage_room = correct(vols_room, TOL)
    voltage_cryo = correct(vols_cryo, TOL)

    pressure_room = 10 ** (voltage_room * 3 - 10)
    pressure_cryo = 10 ** (voltage_cryo * 3 - 10)

    return pressure_room, pressure_cryo
...
def get_log_text():
    ...
    pressure_room, pressure_cryo = get_pressure()
    ...
    return str_room, str_cryo, str_rough
...
while True:
    ...
    with open(path_room + current_date + '_chamber.log', 'a') as file_room, open(path_cryo + current_date + '_dewar.log', 'a') as file_cryo, open(path_rough + current_date + '_foreline.log', 'a') as file_rough:
        ...
        while True:
            ...
            if date == current_date:
                str_room, str_cryo, str_rough = get_log_text()
